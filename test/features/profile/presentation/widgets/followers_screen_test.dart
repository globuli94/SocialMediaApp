// test/features/profile/presentation/widgets/followers_screen_test.dart
//
// Widget tests for FollowersScreen — verifies loading, empty state, populated
...
// we mirror that by wrapping FollowersScreen in a BlocProvider in tests.
...
import 'package:social_network/features/profile/presentation/screens/followers_screen.dart';
...
/// Pumps FollowersScreen via a GoRouter so that context.push() works.
...
Widget _buildSubject({
  ...
  final router = GoRouter(
    initialLocation: '/profile/$uid/followers',
    routes: [
      GoRoute(
        path: '/profile/:uid',
        builder: (_, state) => BlocProvider<FollowListBloc>.value(
          value: followListBloc,
          child: FollowersScreen(uid: state.pathParameters['uid']!),
        ),
        routes: [
          GoRoute(
            path: 'followers',
      ...
      GoRoute(
        path: '/profile/:uid2',
        builder: (_, state) => Scaffold(
          body: Text('Profile ${state.pathParameters['uid2']}'),
        ),
      ),
    ],
  );
...
void main() {
  ...
  group('Empty state', () {
    testWidgets('shows "No followers yet" when list is empty',
        (tester) async {
      when(() => followListBloc.state)
  ...
  group('Populated list', () {
    ...
    testWidgets('shows a ListTile for each follower', (tester) async {
      await tester.pumpWidget(_buildSubject(followListBloc: followListBloc));
      await tester.pump();
    ...
    testWidgets('shows display names of followers', (tester) async {
      await tester.pumpWidget(_buildSubject(followListBloc: followListBloc));
      await tester.pump();
    ...
    testWidgets('shows a leading widget (avatar) for each follower',
        (tester) async {
      await tester.pumpWidget(_buildSubject(followListBloc: followListBloc));
      await tester.pump();

      // Each ListTile should have a leading widget (AvatarWidget).
  ...
  group('Navigation', () {
    ...
    testWidgets('tapping a follower entry navigates to their profile',
        (tester) async {
      // Single /profile/:uid route. For 'uid-me' it renders FollowersScreen;
      // for any other uid it renders a stub "Profile <uid>" so we can detect
      // navigation to the tapped user's profile.
      final router = GoRouter(
        initialLocation: '/profile/uid-me/followers',
        routes: [
          GoRoute(
            path: '/profile/:uid',
            builder: (_, state) {
  ...
  group('Error state', () {
    testWidgets('shows "Could not load followers" when FollowListFailure',
        (tester) async {
      when(() => followListBloc.state)
  ...
  testWidgets('shows "Followers" in AppBar title', (tester) async {
    when(() => followListBloc.state)
        .thenReturn(const FollowListLoaded(users: []));
