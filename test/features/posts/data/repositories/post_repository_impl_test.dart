...
void main() {
  ...
  // -------------------------------------------------------------------------
  // watchPostsByAuthor (FEAT-008)
  // -------------------------------------------------------------------------

  group('watchPostsByAuthor', () {
    test('emits empty list when author has no posts', () async {
      final result = await sut.watchPostsByAuthor('uid-nobody').first;
      expect(result, isEmpty);
    });

    test('emits only posts by the specified author', () async {
      final ts1 = Timestamp.fromDate(DateTime(2026, 2, 1));
      final ts2 = Timestamp.fromDate(DateTime(2026, 2, 2));

      await fakeFirestore.collection('posts').doc('p-alice-1').set({
        'id': 'p-alice-1',
        'authorUid': 'uid-alice',
        'authorDisplayName': 'Alice',
      ...
      await fakeFirestore.collection('posts').doc('p-alice-2').set({
        'id': 'p-alice-2',
        'authorUid': 'uid-alice',
        'authorDisplayName': 'Alice',
      ...
      await fakeFirestore.collection('posts').doc('p-bob-1').set({
        'id': 'p-bob-1',
        'authorUid': 'uid-bob',
        'authorDisplayName': 'Bob',
        'content': 'Bob post',
      ...
      final result = await sut.watchPostsByAuthor('uid-alice').first;

      expect(result, hasLength(2));
      expect(result.every((p) => p.authorUid == 'uid-alice'), isTrue);
    });

    test('orders posts newest first (createdAt descending)', () async {
      final older = Timestamp.fromDate(DateTime(2026, 3, 1));
      final newer = Timestamp.fromDate(DateTime(2026, 3, 5));

      await fakeFirestore.collection('posts').doc('p-old').set({
        'id': 'p-old',
        'authorUid': 'uid-alice',
        'authorDisplayName': 'Alice',
        'content': 'Older post',
      ...
      await fakeFirestore.collection('posts').doc('p-new').set({
        'id': 'p-new',
        'authorUid': 'uid-alice',
        'authorDisplayName': 'Alice',
        'content': 'Newer post',
    ...
    test('maps PostEntity fields correctly', () async {
      final ts = Timestamp.fromDate(DateTime(2026, 4, 10));
      await fakeFirestore.collection('posts').doc('p-mapped').set({
        'id': 'p-mapped',
        'authorUid': 'uid-alice',
        'authorDisplayName': 'Alice',
      ...
      expect(entity.content, 'Mapped content');
